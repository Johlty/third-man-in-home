...
int main()
{
	...
	while (x <= xk)
	{
		...
		a = 2 * ((x - 1) / (x + 1)); // вираз залежить від умови завдання варіанту
		S = a;
		do {
			n++;
			R = ((2 * n - 1) / (2 * n + 1)) * (pow(x - 1, 2 * n + 1) / pow(x - 1, 2 * n - 1)) * (pow(x + 1, 2 * n + 1) / pow(x + 1, 2 * n + 1)); // вираз залежить від умови завдання варіанту
			a *= R;
			S = 2 * (S + a);
		} while (abs(a) >= eps);
